...
class TestSqlQuery:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param("127.18.0.2:8364", None, id="url_without_public_key"),
        ],
    )
    ...
    def test_get_query_str():
        assert SqlQuery._get_query_str([None, "test", None, "this"]) == "test\nthis"
    ...
    def test_requires_tls_public_key(
        ...
        mock_connection.exasol_db_version = db_version
        assert sql_query._requires_tls_public_key() == expected
    ...
    @staticmethod
    @pytest.mark.parametrize(
        "encryption,expected", [(False, "http://"), (True, "https://")]
    )
...
class TestImportQuery:
    ...
    def test_get_import(import_sql_query, columns, expected):
        ...
        assert import_sql_query._get_import(table="TABLE") == expected
...
class TestExportQuery:
    ...
    @staticmethod
    @pytest.mark.parametrize(
        "value,expected", [(True, "WITH COLUMN NAMES"), (False, None)]
    )
