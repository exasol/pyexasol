...
| `encryption` | `True` | Use SSL to encrypt client-server communications for WebSocket and HTTP transport (Default: `False`) |
...
| `encryption` | `True` | Use SSL to encryption for HTTP transport, must be the same as `encryption` of main connection (Default: `False`) |
